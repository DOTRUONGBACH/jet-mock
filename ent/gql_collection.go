...
func (u *UserQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "idCard":
			if _, ok := fieldSeen[user.FieldIDCard]; !ok {
				selectedFields = append(selectedFields, user.FieldIDCard)
				fieldSeen[user.FieldIDCard] = struct{}{}
			}
		case "dateOfBirth":
		...
		case "memberCard":
			if _, ok := fieldSeen[user.FieldMemberCard]; !ok {
				selectedFields = append(selectedFields, user.FieldMemberCard)
				fieldSeen[user.FieldMemberCard] = struct{}{}
			}
		case "createdAt":
